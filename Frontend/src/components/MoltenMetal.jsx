...
void main() {
  ...
  if (uLightMode) {
    ...
    float body =
      smoothstep(0.075, 0.68, signal);

    float ridge =
      smoothstep(0.42, 0.92, signal);

    vec3 lightCol =
      mix(
        uColor1,
        uColor2,
        smoothstep(0.08, 0.52, signal)
      );
...
const MoltenMetal = ({
  ...
  useEffect(() => {
    ...
    u.uBackgroundColor.value[0] =
      bg[0];

    u.uBackgroundColor.value[1] =
      bg[1];

    u.uBackgroundColor.value[2] =
      bg[2];
  }, [
    color1,
    color2,
